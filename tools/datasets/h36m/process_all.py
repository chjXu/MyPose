...
environ['CDF_LIB'] = 'C:\CDF_Distribution\cdf37_1-dist\lib'
...
def process_view(out_dir, subject, action, subaction, camera):
    subj_dir = path.join(root_path, 'extracted', subject)
...
def process_subaction(subject, action, subaction):
    ...
    # 输出路径
    out_dir = path.join(root_path, 'processed', subject, metadata.action_names[action] + '-' + subaction)
