...
root_path = 'G:\datasets\human3.6m'
...
def extract_all():
  for subject_id in tqdm(subjects, ascii=True):
      out_dir = path.join(root_path, 'extracted', subject_id)
      makedirs(out_dir, exist_ok=True)
      extract_tgz(path.join(root_path, 'archives/Poses_D2_Positions_{}.tgz'.format(subject_id)),
                  path.join(out_dir, 'Poses_D2_Positions'))
      extract_tgz(path.join(root_path, 'archives/Poses_D3_Positions_{}.tgz'.format(subject_id)),
                  path.join(out_dir, 'Poses_D3_Positions')),
      extract_tgz(path.join(root_path,'archives/Poses_D3_Positions_mono_{}.tgz'.format(subject_id)),
                  path.join(out_dir, 'Poses_D3_Positions_mono')),
      extract_tgz(path.join(root_path, 'archives/Poses_D3_Positions_mono_universal_{}.tgz'.format(subject_id)),
                  path.join(out_dir, 'Poses_D3_Positions_mono_universal')),
      extract_tgz(path.join(root_path, 'archives/Videos_{}.tgz'.format(subject_id)),
