...
from metadata import load_h36m_metadata, root_path
metadata = load_h36m_metadata()

makedirs(osp.join(root_path, 'images'), exist_ok=True)
    ...
    for a in action_list:
        for sa in subaction_list:
            for c in camera_list:
                ...
                makedirs(osp.join(root_path,'images', subdir), exist_ok=True)

                fileformat = osp.join(root_path, 'images' + '/' + subdir + '/' + subdir + '_%06d.jpg')
                # embed()

                basename = metadata.get_base_filename('S{:d}'.format(s), '{:d}'.format(a), '{:d}'.format(sa), metadata.camera_ids[c-1])
                videoname = basename + '.mp4'
                subject = 'S' + str(s)
                videopath = osp.join(root_path, 'extracted', subject, 'Videos', videoname)
